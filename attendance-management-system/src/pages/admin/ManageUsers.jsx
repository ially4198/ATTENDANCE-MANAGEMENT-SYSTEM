...
const ManageUsers = () => {
  ...
  const parseCSV = (csvText) => {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      user.academicLevel =
        user.academiclevel || (user.role === "student" ? "100" : "");
      ...
      if (text.trim().startsWith("[")) {
        ...
        users.forEach((user, idx) => {
          ...
          parsedUsers.push({
            ...
            academicLevel:
              user.academicLevel || (role === "student" ? "100" : ""),
          });
        });
      } else {
        ...
        lines.forEach((line, idx) => {
          ...
          if (parts.length < 2) {
            errors.push(
              `Line ${idx + 1}: Format should be name|email|role|department|academicLevel`,
            );
            return;
          }
              ...
              {newUser.role === "student" && (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewUser({ ...newUser, academicLevel: e.target.value })
                    }
                    className="w-full px-4 py-3 rounded-lg border-2 border-gray-300 text-sm focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-200"
                  >
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-5 sm:p-8">
            ...
            <div className="flex flex-wrap gap-2 sm:gap-4 mb-6 border-b-2 border-gray-200">
              <button
                ...
                className={`py-3 px-4 sm:px-6 font-semibold border-b-2 transition-colors ${
                  bulkTab === "csv"
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-3">
                    ...
                    name,email,role,department,academicLevel
                  </p>
                </div>
              </div>
            )}
...
[{"name":"John Doe","email":"john@university.edu","role":"student","department":"CS","academicLevel":"100"}]`}
            ...
            {bulkPreview.length > 0 && (
              <div className="mt-6">
                <p className="font-semibold text-gray-900 mb-3">
                  Preview: {bulkPreview.length} user
                  {bulkPreview.length !== 1 ? "s" : ""} to create
                </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-8">
              ...
              >
                Create {bulkPreview.length} User
